...
$(function () {
  ...
  function minitab(a, b, c) {
    ...
    for (var i = 0; i < c.length; i++) {
      // var row = c[i];
      while (a[i] != 0 && b[j] != 0) {
        var indexMin = indexOfMin(c); // Cherche l'index de minimum de la ligne

        console.log('indexMin : ' + indexMin.ligne + " colonne :" + indexMin.colonne);

        for (var j = 0; j < c[indexMin.ligne].length; j++) {
          if (j == indexMin.colonne) {
            if (a[i] < b[j]) {
              x[i][j] = a[i];
              a[i] = 0;
              b[j] -= x[i][j];
  ...
  function solutionDeBase(x, c, a, b) {
    ...
    $("#tableBaseSolution tbody tr").each(function () {
      $(this).find("td").each(function () {
        if (x[i][j] == 0) $(this).html('-');else $(this).html(x[i][j]);
        j++;
      });
      i++;
      j = 0;
  ...
  function indexOfMin(tableau) {
    ...
    var min = tableau[0][0]; // for(var i=0; b[i]==0; i++) {
    //     index++;
    // }
